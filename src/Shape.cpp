...
void Shape::addBlendPos(vector<float> blendBuf, double t)
{
	for (long i = 0; i < posBuf.size(); i++) {
		posBuf[i] += t * blendBuf[i];
	}
}

void Shape::addBlendNor(vector<float> blendBuf, double t)
{
	for (long i = 0; i < posBuf.size(); i++) {
		norBuf[i] += t * blendBuf[i];
	}
}

void Shape::addBlendTex(vector<float> blendBuf, double t)
{
	for (long i = 0; i < posBuf.size(); i++) {
		texBuf[i] += t * blendBuf[i];
	}
}
